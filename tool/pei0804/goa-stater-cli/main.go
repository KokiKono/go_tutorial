...
import (
	"fmt"
	"github.com/KokiKono/go_tutorial/client"
	"github.com/KokiKono/go_tutorial/tool/cli"
...
func main() {
	...
	app := &cobra.Command{
		Use:   "pei0804/goa-stater-cli",
		Short: `CLI client for the pei0804/goa-stater service ()`,
	}
	...
	adminAuthSigner := newAdminAuthSigner(key, format)
	generalAuthSigner := newGeneralAuthSigner(key, format)

	// Initialize API client
	c.SetAdminAuthSigner(adminAuthSigner)
	c.SetGeneralAuthSigner(generalAuthSigner)
	c.UserAgent = "pei0804/goa-stater-cli/0"
...
// newAdminAuthSigner returns the request signer used for authenticating
// against the adminAuth security scheme.
func newAdminAuthSigner(key, format string) goaclient.Signer {
...
// newGeneralAuthSigner returns the request signer used for authenticating
// against the generalAuth security scheme.
func newGeneralAuthSigner(key, format string) goaclient.Signer {
	return &goaclient.APIKeySigner{
		SignQuery: false,
		KeyName:   "X-Authorization",
		KeyValue:  key,
